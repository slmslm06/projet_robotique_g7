// ============================================================
//  PARCOURS COMPLET v10
//
//  CHANGEMENTS vs v9 :
//   - TUNNEL : REMPLACE par la version "suivi du mur droit"
//              (servo a droite, ultrason mesure dist mur droit,
//               correction proportionnelle, sortie sur ligne)
...
// ============================================================
// TUNNEL v10 - NOUVELLE VERSION (suivi du mur droit)
// ============================================================
// Servo bloque a droite ; on mesure la distance au mur droit.
// Cible : ~26 cm du mur droit (centre du tunnel = 25 cm).
// Sortie du tunnel : retour de la ligne sur les capteurs.
#define TUNNEL_TIMEOUT          30000UL
#define TUNNEL_DIST_CIBLE_MIN   24
#define TUNNEL_DIST_CIBLE_MAX   28
#define TUNNEL_VIT_AVANCE       23
#define TUNNEL_VIT_CORR_FORT    35
#define TUNNEL_VIT_CORR_FAIBLE   8
...
void setup() {
  ...
  Serial.println(F("=== PARCOURS COMPLET v10 ==="));
...
// ============================================================
// TUNNEL v10 - SUIVI DU MUR DROIT (remplace v9)
//
// Principe :
//   - Servo bloque sur SERVO_DROITE (90 deg vers la droite)
//   - Ultrason mesure la distance au mur droit en continu
//   - Cible : 24..28 cm du mur droit
//     * < 24 : trop pres -> braquer a gauche
//     * > 28 : trop loin -> braquer a droite
//     * sinon : avance droite
//   - Sortie : retour de la ligne (capteurs != 0b0000) ou timeout
// ============================================================
// Etat TUNNEL : traversee guidee par l'ultrason (detail dans le corps).
// On sort de cet etat des que la ligne est retrouvee.
void gererTunnel() {
  Serial.println(F("\n=== TUNNEL v10 - SUIVI MUR DROIT ==="));

  // Positionner le servo a droite et attendre qu'il s'y mette
  servoUs.write(SERVO_DROITE);
  delay(300);
  ...
  while (millis() - debutTunnel < TUNNEL_TIMEOUT) {
    // 1) Verifier ligne au sol = sortie tunnel
    uint8_t e = lireCapteurs();
    if (e != 0b0000) {
      ...
      piloterMoteur(MOTEUR_D, FREIN, 0);
      delay(150);
      // Remettre le servo en avant pour la suite du parcours
      servoUs.write(SERVO_AVANT);
      delay(200);
      dernierEtatCapteurs = e;
      tunnelFait = true;
    ...
    // 2) Mesure distance mur droit (servo deja en place)
    long distD = mesureUltrason();
    ...
    if (distD < TUNNEL_DIST_CIBLE_MIN) {
      // trop pres du mur droit -> braquer GAUCHE
      avancer(TUNNEL_VIT_CORR_FAIBLE, TUNNEL_VIT_CORR_FORT);
    } else if (distD > TUNNEL_DIST_CIBLE_MAX) {
      // trop loin du mur droit -> braquer DROITE
      avancer(TUNNEL_VIT_CORR_FORT, TUNNEL_VIT_CORR_FAIBLE);
    } else {
      // bonne zone : avance droite
      avancer(TUNNEL_VIT_AVANCE, TUNNEL_VIT_AVANCE);
    }
...
// ============================================================
// EVITEMENT GAUCHE (O1) - inchange par rapport a v9
// ============================================================
