// ============================================================
//  PARCOURS COMPLET v8
//   - TUNNEL : pivot SYSTEMATIQUE sur place avant chaque pas
//     pour bien recentrer le robot, puis avance en ligne droite
//   - EVITEMENT O1 : pivot 70 gauche apres detection ligne
//   - EVITEMENT O2 : pivot 70 droite apres detection ligne
//     (pour s'aligner vers la rampe, pas vers V4)
// ============================================================
...
// ============================================================
// TUNNEL v8 - parametres
// Tunnel = 50 cm large, robot ~18 cm
// Centre = ~16 cm du mur de chaque cote
// ============================================================
#define TUNNEL_TIMEOUT          30000UL
#define MUR_DIST_CIBLE          16    // cm : centre du tunnel
#define MUR_DIST_TOLERANCE      4     // erreur acceptable
#define MUR_DIST_MAX_VALIDE     55    // au-dela = pas de mur

// Durees de pivot sur place (vitesse VITESSE_TUNNEL)
// A vitesse 18, un pivot 90 prend environ 1500 ms
// Donc pour un pivot de 10 degres, environ 170 ms
#define PIVOT_TUNNEL_COURT      120   // ~7 deg, erreur faible
#define PIVOT_TUNNEL_MOYEN      220   // ~13 deg, erreur moyenne
#define PIVOT_TUNNEL_LONG       350   // ~21 deg, erreur grande
#define PAS_AVANCE_TUNNEL       250   // duree avance ligne droite
...
void setup() {
  ...
  Serial.println(F("=== PARCOURS COMPLET v8 ==="));
...
// ============================================================
// TUNNEL v8 - PIVOT SYSTEMATIQUE SUR PLACE AVANT AVANCE
//
// Principe : on DISSOCIE rotation et translation.
//   1) Arret total
//   2) Lecture ligne au sol (sortie si trouvee)
//   3) Scan AVANT
//      - Si tous murs lointains -> sortie tunnel
//      - Si mur devant proche -> virage en place jusqu'a degager
//   4) Scan GAUCHE puis Scan DROITE
//   5) ORIENTATION par PIVOT SUR PLACE :
//      - Calcul de l'erreur (distG - distD si 2 murs, ou
//        distance vs cible 16 cm si 1 mur)
//      - Si |erreur| > tolerance : pivot court vers cote plus libre
//        durée proportionnelle a l'erreur
//      - Sinon : centre, on passe a l'avance
//   6) AVANCE EN LIGNE DROITE pendant un court pas (250 ms)
//   7) Retour boucle
// ============================================================
// Etat TUNNEL : traversee guidee par l'ultrason (detail dans le corps).
// On sort de cet etat des que la ligne est retrouvee.
void gererTunnel() {
  Serial.println(F("\n=== TUNNEL v8 - PIVOT SYSTEMATIQUE ==="));
  ...
  while (millis() - debutTunnel < TUNNEL_TIMEOUT) {
    ...
    // ============================================================
    // 8) CENTRAGE PAR PIVOT SUR PLACE
    //    Calcule l'erreur et pivote sur place vers le cote plus libre
    // ============================================================
    long erreur = 0;
    ...
    if (deuxMurs) {
      // erreur > 0 -> robot plus proche du mur DROIT -> doit pivoter A GAUCHE
      // erreur < 0 -> robot plus proche du mur GAUCHE -> doit pivoter A DROITE
      erreur = distG - distD;
      Serial.print(F("  Deux murs, erreur="));
      Serial.println(erreur);
    } else if (murG_seul) {
      // un seul mur a gauche : viser MUR_DIST_CIBLE (16 cm)
      // si distG < cible -> trop pres du mur G -> pivoter DROITE (erreur negative)
      // si distG > cible -> trop loin du mur G -> pivoter GAUCHE (erreur positive)
      erreur = -(MUR_DIST_CIBLE - distG);   // <0 si trop pres G, >0 si trop loin
      // Reformule : on veut le robot a 16 du mur G, donc :
      //   distG < 16 -> trop pres -> doit s'ecarter (pivoter droite) -> erreur negative
      //   distG > 16 -> trop loin -> doit se rapprocher (pivoter gauche) -> erreur positive
      // Mais notre convention "deux murs" : erreur >0 = pivoter gauche, donc :
      erreur = distG - MUR_DIST_CIBLE;   // >0 si trop loin -> pivot gauche ; <0 si trop pres -> pivot droite
      Serial.print(F("  Mur G seul (distG="));
      Serial.print(distG);
      Serial.print(F("), erreur="));
      Serial.println(erreur);
    } else if (murD_seul) {
      // un seul mur a droite : symetrique
      // distD < 16 -> trop pres D -> doit s'ecarter (pivoter gauche) -> erreur positive
      // distD > 16 -> trop loin D -> doit se rapprocher (pivoter droite) -> erreur negative
      erreur = MUR_DIST_CIBLE - distD;
      Serial.print(F("  Mur D seul (distD="));
      Serial.print(distD);
      Serial.print(F("), erreur="));
      Serial.println(erreur);
    } else {
      // Aucun mur lateral : pas de pivot, on avance simplement
      Serial.println(F("  Aucun mur lateral - avance simple"));
    }

    // ============ 9) PIVOT SUR PLACE selon l'erreur ============
    ...
    if (absErr > MUR_DIST_TOLERANCE) {
      ...
      if (erreur > 0) {
        // erreur positive -> pivoter A GAUCHE
        ...
        // erreur negative -> pivoter A DROITE
      ...
      // Apres le pivot, on REPREND la boucle du haut pour re-scanner
      // et confirmer que l'orientation est meilleure
      continue;
    }

    // ============ 10) BIEN CENTRE -> AVANCE LIGNE DROITE ============
    Serial.println(F("  CENTRE -> avance"));
    avancer(VITESSE_TUNNEL, VITESSE_TUNNEL);
    delay(PAS_AVANCE_TUNNEL);
    // Retour boucle (arret + nouveau scan)
  }
...
// ============================================================
// EVITEMENT GAUCHE v8 - PIVOT 70 GAUCHE pour aligner sur L3
// (apres contournement, robot perpendiculaire a L3 ;
//  pivot 70 gauche pour repartir dans le sens de la ligne)
// ============================================================
...
void evitementGauche() {
  ...
  if (chercherLigne(5000)) {
    ...
    // PIVOT 70 GAUCHE pour s'aligner avec L3 (sens normal de la ligne)
...
// ============================================================
// EVITEMENT DROITE v8 - PIVOT 70 DROITE pour aligner sur L5
// (apres contournement, robot perpendiculaire a L5 ;
//  pivot 70 DROITE pour repartir VERS LA RAMPE, pas vers V4)
// ============================================================
