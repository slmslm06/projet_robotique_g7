// ============================================================
//  PARCOURS COMPLET v9
//   - TUNNEL : refonte v9 avec
//       * mode URGENT (pivot fort si mur lateral < 8 cm)
//       * servo a 135 / 45 deg pour capter murs en biais
//       * logique de virage frontal corrigee
//   - EVITEMENT O1 : pivot final RETIRE (suivi de ligne gere)
//   - EVITEMENT O2 : pivot 70 droite conserve (fonctionnait bien)
// ============================================================
...
// NOUVEAU v9 : angles en biais pour tunnel
#define SERVO_BIAIS_G  135   // gauche-avant (45 deg vers la gauche)
#define SERVO_BIAIS_D  45    // droite-avant (45 deg vers la droite)

#define VITESSE_BASE      40
#define VITESSE_LENTE     30
#define VITESSE_TUNNEL    18
...
#define DUREE_TOURNE_70    680    // NOUVEAU - pivot d'alignement post-evitement
...
#define DIST_DEPASSEMENT2  30
#define DIST_OBST_CIBLE    30
#define DIST_OBST_PROCHE   60
#define DUREE_MAX_LONGER   8000UL

// ============================================================
// TUNNEL v9 - parametres
// Tunnel = 50 cm large, robot ~18 cm
// Centre = ~16 cm du mur de chaque cote
// Ultrason en BIAIS 45 deg : si robot bien aligne et a 16 cm
//   d'un mur, l'ultrason a 135 deg mesure ~16/cos(45) = ~23 cm
//   du mur de cote en realite (mais c'est OK, l'important
//   est la SYMETRIE entre G et D)
// ============================================================
#define TUNNEL_TIMEOUT          30000UL
#define MUR_DIST_CIBLE_BIAIS    23    // cm : centre du tunnel vu en biais
#define MUR_DIST_TOLERANCE      4     // erreur acceptable
#define MUR_DIST_MAX_VALIDE     70    // au-dela = pas de mur

// MODE URGENT : si un mur lateral devient TRES proche
// (en mesure perpendiculaire ~5 cm correspond a ~7 cm en biais 45)
#define MUR_URGENT_BIAIS        10    // cm en biais = ~7 cm reels
#define PIVOT_URGENT_DUREE      350   // pivot fort pour s'eloigner

// Durees de pivot sur place (vitesse VITESSE_TUNNEL)
#define PIVOT_TUNNEL_COURT      120
#define PIVOT_TUNNEL_MOYEN      220
#define PIVOT_TUNNEL_LONG       350
#define PAS_AVANCE_TUNNEL       250
...
void setup() {
  ...
  Serial.println(F("=== PARCOURS COMPLET v9 ==="));
...
// ============================================================
// TUNNEL v9 - PIVOT SYSTEMATIQUE + MODE URGENT + SERVO EN BIAIS
//
// Principe :
//   1) Arret total
//   2) Lecture ligne au sol (sortie si trouvee)
//   3) Scan AVANT, puis G en biais 135, puis D en biais 45
//   4) Sortie si tous murs lointains
//   5) URGENT : si UN mur lateral < 10 cm en biais
//      -> pivot fort dans le sens oppose (350 ms) -> recommencer
//   6) MUR DEVANT < 25 cm
//      -> choisir sens du virage selon mur LATERAL le plus proche
//         (s'eloigner du mur le plus proche, pas du mur frontal)
//      -> pivoter jusqu'a degager
//   7) CENTRAGE par equilibrage distG vs distD
//      - erreur = distG - distD (les deux en biais)
//      - Si |erreur| > 4 -> pivot proportionnel
//      - Sinon -> on avance en ligne droite (250 ms)
// ============================================================
// Etat TUNNEL : traversee guidee par l'ultrason (detail dans le corps).
// On sort de cet etat des que la ligne est retrouvee.
void gererTunnel() {
  Serial.println(F("\n=== TUNNEL v9 - SERVO BIAIS + URGENT ==="));
  ...
  while (millis() - debutTunnel < TUNNEL_TIMEOUT) {
    ...
    // ============ 4) SCAN GAUCHE en BIAIS 135 ============
    servoUs.write(SERVO_BIAIS_G);
    delay(300);
    long distG = mesureUltrason();

    // ============ 5) SCAN DROITE en BIAIS 45 ============
    servoUs.write(SERVO_BIAIS_D);
    delay(350);
    long distD = mesureUltrason();
    ...
    // ============================================================
    // 7) MODE URGENT : un mur lateral est TRES proche
    //    Pivot fort dans le sens oppose, puis re-scan
    // ============================================================
    if (distG > 0 && distG < MUR_URGENT_BIAIS) {
      Serial.print(F("  !! URGENT mur G tres proche ("));
      Serial.print(distG);
      Serial.println(F(") -> pivot DROITE fort"));
      // pivot droite = G ARRIERE + D ARRIERE
      piloterMoteur(MOTEUR_G, ARRIERE, VITESSE_TUNNEL);
      piloterMoteur(MOTEUR_D, ARRIERE, VITESSE_TUNNEL);
      delay(PIVOT_URGENT_DUREE);
      piloterMoteur(MOTEUR_G, FREIN, 0);
      piloterMoteur(MOTEUR_D, FREIN, 0);
      delay(100);
      continue;
    }
    if (distD > 0 && distD < MUR_URGENT_BIAIS) {
      Serial.print(F("  !! URGENT mur D tres proche ("));
      Serial.print(distD);
      Serial.println(F(") -> pivot GAUCHE fort"));
      // pivot gauche = G AVANT + D AVANT
    ...
    // ============================================================
    // 8) MUR DEVANT : virage
    //    Choix du sens base sur l'EQUILIBRE des cotes (distG/distD)
    //    PAS sur le mur frontal lui-meme.
    // ============================================================
    if (distAvant > 0 && distAvant < DIST_MUR_FRONTAL) {
      Serial.println(F("MUR DEVANT - virage"));
      // On vire du cote ou il y a PLUS d'espace lateral
      // Si distG > distD -> plus d'espace a gauche -> virer GAUCHE
      // Si distD > distG -> plus d'espace a droite -> virer DROITE
      bool virerGauche = (distG > distD);
      Serial.print(F("  G(b)="));
      Serial.print(distG);
      Serial.print(F(" D(b)="));
      Serial.print(distD);
      Serial.println(virerGauche ? F(" -> virer GAUCHE") : F(" -> virer DROITE"));
    ...
    // ============================================================
    // 9) CENTRAGE PAR PIVOT SUR PLACE
    //    erreur > 0 (distG > distD) -> plus loin du mur G,
    //               donc plus pres du mur D -> pivoter GAUCHE
    //    erreur < 0 (distD > distG) -> plus loin du mur D,
    //               donc plus pres du mur G -> pivoter DROITE
    // ============================================================
    long erreur = 0;
    bool deuxMurs = (distG < MUR_DIST_MAX_VALIDE && distD < MUR_DIST_MAX_VALIDE);
    bool murG_seul = (distG < MUR_DIST_MAX_VALIDE && distD >= MUR_DIST_MAX_VALIDE);
    bool murD_seul = (distD < MUR_DIST_MAX_VALIDE && distG >= MUR_DIST_MAX_VALIDE);

    if (deuxMurs) {
      // erreur > 0 -> mur G plus loin -> on est plus pres du mur D -> pivoter GAUCHE
      // erreur < 0 -> mur D plus loin -> on est plus pres du mur G -> pivoter DROITE
      erreur = distG - distD;
      Serial.print(F("  Deux murs, erreur="));
      Serial.println(erreur);
    } else if (murG_seul) {
      // Un seul mur G visible : viser MUR_DIST_CIBLE_BIAIS (23 cm en biais)
      // distG < 23 -> trop pres mur G -> pivoter DROITE (erreur negative)
      // distG > 23 -> trop loin mur G -> pivoter GAUCHE (erreur positive)
      erreur = distG - MUR_DIST_CIBLE_BIAIS;
      Serial.print(F("  Mur G seul (distG="));
      Serial.print(distG);
      Serial.print(F("), erreur="));
      Serial.println(erreur);
    } else if (murD_seul) {
      // Un seul mur D visible : symetrique
      // distD < 23 -> trop pres mur D -> pivoter GAUCHE (erreur positive)
      // distD > 23 -> trop loin mur D -> pivoter DROITE (erreur negative)
      erreur = MUR_DIST_CIBLE_BIAIS - distD;
      Serial.print(F("  Mur D seul (distD="));
      Serial.print(distD);
      Serial.print(F("), erreur="));
      Serial.println(erreur);
    } else {
      Serial.println(F("  Aucun mur lateral - avance simple"));
    }

    // ============ 10) PIVOT SUR PLACE selon erreur ============
    long absErr = (erreur < 0) ? -erreur : erreur;
    
    if (absErr > MUR_DIST_TOLERANCE) {
      unsigned long dureePivot;
      if (absErr > 20) {
        dureePivot = PIVOT_TUNNEL_LONG;
      } else if (absErr > 10) {
        dureePivot = PIVOT_TUNNEL_MOYEN;
      } else {
        dureePivot = PIVOT_TUNNEL_COURT;
      }

      if (erreur > 0) {
        // pivoter GAUCHE
        Serial.print(F("  >> PIVOT GAUCHE "));
        Serial.print(dureePivot);
        Serial.println(F(" ms"));
        piloterMoteur(MOTEUR_G, AVANT, VITESSE_TUNNEL);
        piloterMoteur(MOTEUR_D, AVANT, VITESSE_TUNNEL);
      } else {
        // pivoter DROITE
        Serial.print(F("  >> PIVOT DROITE "));
        Serial.print(dureePivot);
        Serial.println(F(" ms"));
        piloterMoteur(MOTEUR_G, ARRIERE, VITESSE_TUNNEL);
        piloterMoteur(MOTEUR_D, ARRIERE, VITESSE_TUNNEL);
    ...
    // ============ 11) BIEN CENTRE -> AVANCE LIGNE DROITE ============
    Serial.println(F("  CENTRE -> avance"));
    avancer(VITESSE_TUNNEL, VITESSE_TUNNEL);
    delay(PAS_AVANCE_TUNNEL);
  }
...
// ============================================================
// EVITEMENT GAUCHE v9 - PAS de pivot final
// (le suivi de ligne s'occupe de redresser le robot)
// ============================================================
...
void evitementGauche() {
  ...
  if (chercherLigne(5000)) {
    Serial.println(F("Ligne trouvee - pas de pivot final, suivi de ligne prend le relais"));
  } else {
...
// ============================================================
// EVITEMENT DROITE v9 - PIVOT 70 DROITE conserve
// (fonctionne bien : aligne le robot vers la rampe)
// ============================================================
...
void evitementDroite() {
  ...
  if (chercherLigne(5000)) {
    ...
    // PIVOT 70 DROITE pour s'aligner vers la rampe (et non vers V4)
